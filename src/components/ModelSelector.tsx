...
export function ModelSelector({ selectedModel, onSelect, currentPlan, onUpgradeRequest }: ModelSelectorProps) {
  ...
  const handleSelect = (model: Model) => {
    if (!canAccessModel(currentPlan, model.tier)) { onUpgradeRequest(); setOpen(false); return; }
    onSelect(model.id);
    setOpen(false);
  };
  ...
  const Section = ({ title, badge, models }: { title: string; badge?: string; models: Model[] }) => (
    <div className="border-t border-white/10 p-2">
      ...
      {models.map((model) => {
        ...
        return (
          <button key={model.id} onClick={() => handleSelect(model)} className={`flex w-full items-center gap-2.5 rounded-xl px-3 py-2 text-left transition-all ${selected.id === model.id ? "bg-white text-black" : hasAccess ? "text-gray-300 hover:bg-white/10" : "text-gray-500 hover:bg-white/5"}`}>
            <span className={`text-base ${model.iconColor}`}>{model.icon}</span>
            <span className="text-sm">{model.name}</span>
            {!hasAccess && <span className="ml-auto rounded-full bg-white/5 px-2 py-0.5 text-[9px] font-bold text-gray-600">{model.tier.toUpperCase()}</span>}
  ...
  return (
    <div ref={ref} className="relative">
      <button onClick={() => setOpen(!open)} className="flex items-center gap-2 rounded-full border border-white/20 bg-[#2f2f2f] px-4 py-2 text-sm text-white transition-all hover:bg-[#3a3a3a]">
          ...
          <div className="max-h-[60vh] overflow-y-auto">
            <div className="p-2">
              ...
              {MODELS.filter((m) => m.tier === "free").map((model) => (
                <button key={model.id} onClick={() => handleSelect(model)} className={`flex w-full items-center gap-2.5 rounded-xl px-3 py-2 text-left transition-all ${selected.id === model.id ? "bg-white text-black" : "text-gray-300 hover:bg-white/10"}`}>
                  <span className={`text-base ${model.iconColor}`}>{model.icon}</span>
                  <span className="text-sm">{model.name}</span>
                  {selected.id === model.id && <svg className="ml-auto h-4 w-4 text-black" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" /></svg>}
                </button>
